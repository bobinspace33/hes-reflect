...
import { SITE_CLOSING_COMMENTARY_KEY } from "@/lib/site-copy";

export function SiteClosingCommentaryEditor({ initialBody }: { initialBody: string }) {
  ...
  async function save() {
    ...
    try {
      const res = await fetch("/api/admin/site-strings", {
        ...
        body: JSON.stringify({ key: SITE_CLOSING_COMMENTARY_KEY, body: text }),
      });
  ...
  return (
    <div className="rounded-2xl border border-silver-700 bg-silver-800/40 overflow-hidden p-5">
      <div className="flex items-start justify-between gap-4 mb-3">
        <div>
          <h3 className="font-medium text-silver-50">Closing commentary</h3>
          <p className="text-silver-300/55 text-xs font-mono mt-1 max-w-xl">
            Shown in the Closing commentary panel on the public site — same slide-over behavior as
            Introduction, editable here without touching code.
          </p>
        </div>
        <button
          type="button"
      ...
      <textarea
        ...
        rows={12}
        placeholder="Final thoughts, acknowledgments, where to go next…"
